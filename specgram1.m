function [yo,fo,to] = specgram1(x, nfft, Fs, windowSize, noverlap)
...
% [x,nfft,Fs,window,noverlap]=specgramchk1(varargin);%!!!!!!!!!!!!!!!!!separate varargin into 5 components

% display(window);
% coder.extrinsic('specWindow');
if length(windowSize) == 1 
    temp_n = 0:windowSize-1;
    window = 0.5 * (1 - cos(2*pi*temp_n/windowSize));
end
...
%     end
    ...
    yy = upfirdn(x,D,1,q).';
    yy(:,[1:extras+1 end-extras+1:end]) = []; 
end
