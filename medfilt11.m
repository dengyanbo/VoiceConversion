function y = medfilt11(x,n,blksz,DIM)
...
% if isempty(DIM)
    ...
    for i = 1:prod(siz(2:end)),
        y(:,i) = medfilt1D(xx(:,i),n,blksz);
    end
    y = shiftdim(y, -nshifts);
% else
% 	% Put DIM in the first (row) dimension (this matches the order 
% 	% that the built-in filter function uses)
%     coder.extrinsic('permuteHelper');
% 	perm = [DIM,1:DIM-1,DIM+1:ndims(x)];
%     display(perm);
%     display(length(x));
%     x = permuteHelper(x, perm);
% 	x = permute(x,tempPerm);
%     error(message('permute unavailable'));
