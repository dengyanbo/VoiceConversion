...
% coder.extrinsic('nonlinearFilt');
% tempData = zeros(length(DataA),1);

tempData = nonlinearFilt(w1, w2, Filter_order, DataA);
...
tempData = nonlinearFilt(w1, w2, Filter_order, DataC);
LenData_dec = fix ((lenDataA+dec_factor-1)/dec_factor);
DataD = zeros(LenData_dec, 1);
