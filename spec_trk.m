...
% coder.extrinsic('spec_trkWindow');
% Kaiser_window = zeros(nframesize, 1);
Kaiser_window = spec_trkWindow(nframesize)';
SHC = zeros(1,max_SHC);

for frame = 1:numframes
    if (VUVEnergy(frame) > 0)
        firstp = 1+(frame-1)*(nframejump);
        SignalTemp = Data(firstp:firstp+nframesize-1);
        Signal = SignalTemp .* Kaiser_window;
...
VPeak_minmrt = medfilt11(VPeak_minmrt, max(1,Prm.median_value-2));
...
VPitch = medfilt11(VPitch, max(Prm.median_value-2, 1));
